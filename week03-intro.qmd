...
# if you want to take out scientific notation change option(scipen = 0)
options(scipen = 999)
```
...
?mtcars
#help tab about dataset(if they are built-in)
```
...
-   There are 32 rows and 12 columns in the dataframe

    ## What are the variables you see?

    #use function colnames(mtcars)
...
ggplot(data = mtcars, aes(x = drat, y = disp)) + 
  geom_point() +
  geom_smooth()
