...
#rename to penguins filter, pull from penguins df and pipe next function so that we only grab Female penguins with culmen length greater than 39 and reside on dream and torgersen island.

#these filters are all in filter() function and are separated by commas

# == is to set value match, >= means greater than or equal to, and x %in% z means go into column x and grab these cell matches 
```
...
#create new df and call it penguins_select, pull from penguins df, pipe select function to this and either select(-x) select(x)

#select() grabs column x and puts it into our new data frame penguins_select. select(-x) removes column x from the penguins df but keeps everything else
...
#penguins_relocate new df from penguins df and pipes relocate()

#relocate() grabs column individual_id and .after puts it after column sample_number

#so far we know .before and .after for syntax after original column
```
...
#create df penguins_rename from penguins df and rename column island to island_name. New name always comes first
```
...
penguins_mutate <- penguins %>% 
  mutate(flippometers = flipper_length_mm * 0.000001)

#create df penguins_mutate from penguins df and create new column flippometers from existing column flipper_length_mm. new column first thing in function
...
#create df number8 from penguins df, pipe into group by function which looks at unique values from the island and species columns.
#there are no actual visible changes until we use summarize()
# summarize lets us create the culmen_depth_mean and culmen_length_mean columns which let us give a mean to the unique group combinations
...
#across() only works in conjunction with summarize() or mutate(), does not work as a standalone function

#across() works with starts_with() or ends_with() and used to specify classes like is.numeric or is.character

#a few formatting notes:
  #this code chunk tells us that to filter by island Biscoe and group by unique cells in species column, and to summarize across all variables that are numeric type and produce the max value for this island and species combo. na.rm gets rid of any na cells
  #we use where() as a selection helper
  # if not trying to filter by type, common formatting is:
    #summarize(acrossc((column_x, column_y, column_z), mean, na.rm))
#or
    #summarize(across(column_a:column_p, min, na.rm = TRUE))
#or 
...
##YOU WILL GET ERRORS IF YOU HAVE MULTIPLE VALUES AND USE ==
## == only work with one value, if you have 2 values use:
  #filter(column_x %in% c(listed multiple cell values))
...
#summary shows 172 big, 170 small and 2 NA's, which totaled is 344

# just to know ~ or tilde means "then", if-then statement-y

#create df case_when from penguins df and create new column size, and use variables "big" when column body_mass_g is greater or equal to 4050 and "small" when column body_mass_g is less than or equal to 4050
```
